...
train_size = int(len(Y_df3) * 0.69-1)
...
# 生成往前256天的日期
dates = pd.date_range(end=end_date, periods=384, freq='D').date
...
lengthr = len(rv_daily['value']) - 850

# Rolling forecast
for i in range(lengthr):
    # Training set range
    train_start = i  # Start at the very beginning
    train_end = 849 + i  # End at 977 + current step
...
# 保存为 CSV 文件
result_df.to_csv('predictions_nhits_har_384_noexpanding.csv', index=False)
