...
process_mortality_data <- function(file_path) {
  ...
  # 读取数据
  lifetbl_both <- read.table(file_path, skip = 2, header = TRUE, stringsAsFactors = FALSE)
  ...
  lifetbl_both <- lifetbl_both[lifetbl_both$Year < 2020, ]
  lifetbl_male <- lifetbl_male[lifetbl_male$Year < 2020, ]
  lifetbl_female <- lifetbl_female[lifetbl_female$Year < 2020, ]
  ...
  # differ OM_Zhat_t_both
  OM_Zhat_t_both <- diff(OM_Zhat_t_both)
  OM_Zhat_t_male <-diff(OM_Zhat_t_male)
  OM_Zhat_t_female <- diff(OM_Zhat_t_female)
  ...
  OM_Zhat_t0_both <- OM_Zhat_t_both[-(n_periods-1)]
  OM_Zhat_t0_male <- OM_Zhat_t_male[-(n_periods-1)]
  OM_Zhat_t0_female <- OM_Zhat_t_female[-(n_periods-1)]
  ...
  return(list(male_pvalue = OM_unitroottest_p.value_male, female_pvalue = OM_unitroottest_p.value_female , 
              both_pvalue = OM_unitroottest_p.value_both, t_period = n_periods , t_start = 2019 - n_periods +1))
}
