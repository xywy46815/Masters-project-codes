#SET PATH
setwd('C:/Users/taowu/Desktop/country cointegration')
...
lifetbl_both_belgium <- lifetbl_both_belgium[lifetbl_both_belgium$Year >= 1928 & lifetbl_both_belgium$Year < 2020, ]
...
lifetbl_both_Italy <- lifetbl_both_Italy[lifetbl_both_Italy$Year >= 1928 & lifetbl_both_Italy$Year < 2020, ]
...
lifetbl_both_spain <- lifetbl_both_spain[lifetbl_both_spain$Year >= 1928 & lifetbl_both_spain$Year < 2020, ]
...
lifetbl_both_Switzerland <- lifetbl_both_Switzerland[lifetbl_both_Switzerland$Year >= 1928 & lifetbl_both_Switzerland$Year < 2020, ]
...
#z_matrix
# 使用 cbind() 将它们按列绑定为矩阵
z_matrix <- cbind(z_Switzerland_vector, z_spain_vector,  z_Italy_vector,z_Netherlands_vector)#, z_france_vector)
...
t <- 0:(t_last - t_first)
print(t)
df <- data.frame(t, x = z_matrix, y = z_belgium_vector)
