...
for i in range(0, len(test_Y_df3),2):
    ...
    prediction = Y_hat_df['DeepAR-median'].values.flatten()
...
# Length of the dataset minus the number of training observations
lengthr = len(rv_daily['value']) - 1106

# Rolling forecast
for i in range(lengthr):
    # Training set range
    train_start = 0  # Start at the very beginning
    train_end = 1105 + i  # End at 977 + current step
...
# 绘制真实值（测试集）
plt.plot(test_Y_df3['ds'], test_Y_df3['y'], label='real', color='blue', marker='o')

# 绘制预测值
plt.plot(test_Y_df3['ds'], pred['Pred'], label='pred', color='red', linestyle='--', marker='x')
...
# 保存为 CSV 文件
result_df.to_csv('predictions_DeepAR_har_256_noexpanding.csv', index=False)
