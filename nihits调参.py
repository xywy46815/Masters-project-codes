...
train_size = int(len(Y_df3) * 0.7+1)
...
# 参数搜索范围
param_grid = {
    "input_size": [6,12,24,36],
    "dropout_prob_theta": [0.2,0.3],
    "learning_rate": [1e-3, 1e-4],
    "batch_size": [16, 32],
    "activation": ['PReLU','SELU', 'LeakyReLU'],
    "scaler_type":['identity', 'robust', 'standard'],
    "max_steps":[150,200,250],
...
for params in param_combinations:
    input_size,  dropout_prob_theta, learning_rate, batch_size,activation, scaler_type,max_steps,interpolation_mode= params
    models = [NHITS(h=2,
    ...
    for i in range(0, len(test_Y_df3), 2):
        train_Y_df3 = Y_df3.iloc[:train_size + i]  # 扩展训练集
        test_Y_df3 = Y_df3.iloc[train_size + i : train_size + i + 2]  # 测试当前两步

        nf = NeuralForecast(models=models, freq='D')
        nf.fit(df=train_Y_df3)

        Y_hat_df = nf.predict(futr_df=test_Y_df3)
        prediction = Y_hat_df['NHITS-median'].values.flatten()
